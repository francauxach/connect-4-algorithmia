...
function findByDiagonal(matrix, reverse = false) {
...
const diagonalsMatrix = findByDiagonal(matrix)
...
const reversedDiagonalsMatrix = findByDiagonal(reversedMatrix, true)
const reversedDiagonallyMap = reversedDiagonalsMatrix.map((col) => col.join('').includes([...Array(consecutiveOcurrences)].map(() => 'A').join('')) || col.join('').includes([...Array(consecutiveOcurrences)].map(() => 'B').join('')))
